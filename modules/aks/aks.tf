# Node SSH keypair, generated by Terraform rather than read from a local
# file - the previous file("../k8s")/file("../k8s.pub") reads meant
# terraform validate/plan failed on any fresh clone (including CI) since
# those files are gitignored and never committed. The private key is
# stored in Key Vault below for the rare case direct node SSH is needed;
# AKS access day-to-day goes through Azure AD RBAC, not this key.
resource "tls_private_key" "node_ssh" {
  algorithm = "RSA"
  rsa_bits  = 4096
}

resource "azurerm_key_vault_secret" "node_ssh_private_key" {
  name         = "aks-${var.project}-${var.env}-${var.location_short}-node-ssh-key"
  value        = tls_private_key.node_ssh.private_key_pem
